...
class Accuracy(Metric):
    ...
    def compute(self) -> torch.Tensor:
        return self.correct.float() / (self.total + FLOAT32_EPSILON)  # type: ignore
...
class NDCG(Metric):
    ...
    def update(self, preds: torch.Tensor, targets: torch.Tensor) -> None:
        ...
        self.count += 1.0  # type: ignore

    def compute(self) -> torch.Tensor:
        return self.ndcg / self.count  # type: ignore
...
class MRR(Metric):
    ...
    def compute(self) -> torch.Tensor:
        return self.mrr / self.count  # type: ignore
...
class SentiMRR(Metric):
    ...
    def compute(self) -> torch.Tensor:
        return self.senti_mrr / self.count  # type: ignore
...
class PrecisionAtRecall(Metric):
    ...
    def update(self, distances: torch.Tensor, labels: torch.Tensor) -> None:
        ...
        threshold_index = torch.where(torch.cumsum(labels, dim=0) >= self.recall_point * torch.sum(labels))
    ...
    def compute(self) -> torch.Tensor:
        return self.correct.float() / (self.correct + self.wrong + FLOAT32_EPSILON)  # type: ignore
